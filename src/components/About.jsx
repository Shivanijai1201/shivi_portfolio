...
function About() {
  return (
    <section className="relative overflow-hidden bg-paper px-0 py-12" id="about">
      ...
      <h1 className="relative text-center text-5xl font-semibold text-ink max-[576px]:text-4xl" data-aos="fade-up">
        About Me
      </h1>
      ...
      <div className="relative mx-auto mt-12 mb-10 flex w-[90%] max-w-6xl flex-col items-center gap-8 text-center" data-aos="fade-up">
        ...
        <a
          ...
          download=""
          data-aos="fade-up"
          className="group inline-flex w-max items-center gap-2.5 rounded-full bg-primary px-8 py-2 text-sm tracking-widest text-ink uppercase transition-all hover:-translate-y-1 hover:bg-primary-dark hover:text-black hover:shadow-[0_10px_25px_rgba(0,204,255,0.35)]"
        >
          Download CV
        ...
        <div className="grid w-full max-w-2xl grid-cols-3 gap-4" data-aos="fade-up" data-aos-delay="100">
          {STATS.map((stat) => (
            <div
              key={stat.label}
              className="group flex flex-col items-center gap-2 rounded-2xl border border-ink/5 bg-ink/3 px-3 py-5 backdrop-blur-sm transition-all hover:-translate-y-1.5 hover:border-primary/40 hover:bg-ink/5"
            >
              ...
              <span className="text-2xl font-extrabold text-ink">{stat.value}</span>
              <span className="text-center text-xs tracking-wide text-ink/50 uppercase">{stat.label}</span>
            </div>
          ))}
        </div>
      </div>

      <div className="relative w-full bg-paper" id="resume">
        <div className="mx-auto flex w-[90%] max-w-6xl flex-col gap-12 md:flex-row">
          <ResumeBlock heading="Experience" icon="fas fa-briefcase">
            <div
              className="relative rounded-2xl border border-ink/5 border-l-3 border-l-primary bg-ink/3 p-6 backdrop-blur-sm transition-all hover:-translate-y-1 hover:bg-ink/5 hover:shadow-[0_12px_30px_rgba(0,204,255,0.12)]"
              data-aos="fade-up"
            >
              <div className="flex items-start justify-between gap-2.5">
                <h3 className="min-w-0 flex-1 text-lg font-bold text-ink">Software Engineer — Algofolks Private Limited</h3>
              ...
              <ul className="mt-3.5 flex flex-col gap-2">
                {EXPERIENCE_HIGHLIGHTS.map((point) => (
                  <li key={point} className="relative pl-5 text-sm leading-relaxed text-ink/70">
          ...
          <ResumeBlock heading="Education" icon="fas fa-graduation-cap">
            {EDUCATION.map((item, index) => (
              <div
                ...
                className="rounded-2xl border border-ink/5 border-l-3 border-l-primary bg-ink/3 p-6 backdrop-blur-sm transition-all hover:-translate-y-1 hover:bg-ink/5 hover:shadow-[0_12px_30px_rgba(0,204,255,0.12)]"
              >
                <div className="flex items-start justify-between gap-2.5">
                  <h3 className="min-w-0 flex-1 text-lg font-bold text-ink">{item.institution}</h3>
                  <span className="shrink-0 rounded-full border border-primary/25 bg-primary/8 px-2.5 py-0.5 text-xs font-semibold whitespace-nowrap text-primary">
                    {item.location}
                  </span>
                </div>
